...
namespace pgm {
	...
	BeliefPropagation::BeliefPropagation(const CliqueTree& ct) : m_tree(ct) {
	...
	fvallist BeliefPropagation::query(const ProbQueryList& queries) const {
		return queries | std::views::transform(LAMBDA(q, query(q))) | std::ranges::to<fvallist>();
	}

	fval_t BeliefPropagation::query(const ProbQuery& query) const {
		fidlist queryVars;
		...
		//从包含查询变量最多的一个团开始
		fid_t startCliqueId = static_cast<fid_t>(qy::ranges::argmax(m_cliques, {}, [&](auto&& c) {
	...
	void BeliefPropagation::sendMessages() {
		// 先从下往上发消息，割集存在指向的点上
		for (fid_t u : std::views::reverse(m_tree.orderseq)) {
			...
			if (fid_t p = m_tree.nodes[u].parent; p != -1) { //对父结点
				auto elimilatedVars = qy::set_difference<fidlist>(m_cliques[u].getVarIds(),
																  m_cliques[p].getVarIds());
